				...
				Else
					If InStr(strIPAddress, ":") = 0 Or blnShowIPv6 Then
						...
						If Left(cNet(x).IPFormated, 3) = " " Then
							cNet(x).IPFormated = cNet(x).IPFormated & vbTab & vbTab & vbTab & vbTab & vbTab & strIPAddress & strIPSubnet & vbCrLf
						Else
							cNet(x).IPFormated = "IP Address: " & vbTab & strIPAddress + strIPSubnet & vbCrLf
						End If
						If Not IsNull(objItem.DHCPServer) Then
							cNet(x).IPFormated = cNet(x).IPFormated & vbTab & "DHCP Server:   " & objItem.DHCPServer + strDHCP & vbCrLf
						End If
					End If
				End If
			Next
...
For x = 0 to UBound(cNet)
	If cNet(x).NetConnectionStatus <> "Unknown" Then
		sResult = sResult & cNet(x).NetConnectionID & " | " &  cNet(x).MacAddress & vbCrLf
		sResult = sResult & vbTab & cNet(x).NetConnectionStatus & " | " & cNet(x).Speed & vbCrLf 		
